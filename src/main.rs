...
async fn post_register(id: &str, db: Connection<DB>) -> Result<Json<UserData>, Status> {
    ...
    let collection: Collection<UserData> = db
        .database(DATABASE_NAME)
        .collection(&COLLECTIONS[DBCollections::UserStatCollection]);

    let potential_user = collection
        .find_one(
            doc! {
                "steam_id": id
            },
    ...
    .await
    {
        ...
        if data["response"]["players"].is_array()
            && data["response"]["players"].as_array().unwrap().is_empty()
        {
            return Err(Status::NoContent);
        }

        let user_data = UserData {
            steam_id: data["response"]["players"][0]["steamid"]
                .as_str()
                .unwrap()
                .to_string(),
            last_known_username: data["response"]["players"][0]["personaname"]
                .as_str()
                .unwrap()
                .to_string(),
            points: 0,
...
async fn rocket() -> _ {
    ...
    session::setup_hashmap();
    
    info!("Starting Rocket server...");
    rocket::custom(figment)
        .attach(DB::init())
        .mount("/", routes![index, get_data, update_points, post_register, session::get_sessions_by_region, session::list_regions])
